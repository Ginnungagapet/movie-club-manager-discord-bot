...
class MovieManager:
    ...
    def _create_movie_embed(self, movie_details: Dict[str, Any], is_current: bool = False) -> discord.Embed:
        """
        ...
        if genres:
            embed.add_field(name="🎭 Genres", value=', '.join(genres[:3]), inline=True)  # Limit to 3 genres
        ...
        if directors:
            director_names = [director.get('name', str(director)) for director in directors[:2]]
            embed.add_field(name="🎬 Director", value=', '.join(director_names), inline=True)
        ...
        # Add plot
        plot = movie_details.get('plot outline') or (movie_details.get('plot', [''])[0] if movie_details.get('plot') else '')
        if plot:
            # Clean up plot text and truncate
            clean_plot = plot.replace('::Anonymous', '').replace('::IMDb', '').strip()
            truncated_plot = clean_plot[:300] + "..." if len(clean_plot) > 300 else clean_plot
            embed.add_field(name="📖 Plot", value=truncated_plot, inline=False)
        
        # Add footer with IMDB link
        embed.set_footer(text="Click the title to view on IMDB", icon_url="https://m.media-amazon.com/images/G/01/imdb/images/social/imdb_logo.png")
